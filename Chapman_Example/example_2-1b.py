...
dss_text.Command = f"Edit Vsource.source bus1=SourceBus basekv={V_low_line} pu=1.0 Angle=0 BaseFreq=60 MVAsc3=1e12 MVASC1=1e12"

dss_text.Command = "New Transformer.T1 phases=3 windings=2"
dss_text.Command = "~ buses=[SourceBus, HighBus1]"
dss_text.Command = f"~ conns=[wye, wye] kvs=[{V_low_line:.6f}, {V_high_line:.6f}] kvas=[{S_total}, {S_total}]"
dss_text.Command = f"~ XHL={x_percent:.6f} %Rs=[{r_percent:.6f}, 0]"

dss_text.Command = f"New Line.L12 Bus1=HighBus1 Bus2=HighBus2 Phases=3 r1={r_line} x1={x_line} length=1 units=none"

dss_text.Command = "New Transformer.T2 phases=3 windings=2"
dss_text.Command = "~ buses=[HighBus2, LoadBus]"
dss_text.Command = f"~ conns=[wye, wye] kvs=[{V_high_line:.6f}, {V_low_line:.6f}] kvas=[{S_total}, {S_total}]"
dss_text.Command = f"~ XHL={x_percent:.6f} %Rs=[{r_percent:.6f}, 0]"

dss_text.Command = f"New Load.Load Bus1=LoadBus Phases=3 kV={V_low_line} kW={P_Load_total} kvar={Q_Load_total} model=1"
dss_text.Command = f"Set VoltageBases = [{V_high_line}, {V_low_line}]"
